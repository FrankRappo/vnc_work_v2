...
## Модальные окна 1С (T247)

`rc.sh modal…` — опознать модальное окно 1С и нажать в нём кнопку ВОСПРОИЗВОДИМО. Появилось
после того, как на «не нажимается кнопка Да» подряд встали две задачи (T232 §4.1, T246 §8).

```
rc.sh modal [--json]                  какое окно 1С сейчас: заголовок, текст, кнопки
                                      (координаты и DOM, и ЭКРАННЫЕ), поля ввода, точка фокуса
rc.sh modal-click <подпись> [--via x] нажать кнопку с ПРОВЕРКОЙ результата по DOM
rc.sh modal-click --tmpl <эталон>     тонкий клиент: по картинке, CDP не нужен
rc.sh modal-fill <текст>              ввод в поле верхнего окна
rc.sh modal-key <Enter|Escape|…>      клавиша в верхнее окно
env: RC_CDP_PORT — порт отладки браузера с веб-клиентом 1С; RC_DISPLAY — X-дисплей для --via x
```

Три вещи, которых нет в обычном «кликни по координатам» и ради которых всё писалось:
1. **hit-тест перед кликом.** Панель «Сообщения» 1С ложится ПОВЕРХ командной панели окна, и клик
   по геометрическому центру кнопки уходит в неё. Драйвер ищет свободную точку внутри кнопки, а
   если её нет — говорит об этом и называет перекрывающий элемент, вместо клика вслепую.
2. **Вердикт различает «занята» и «не дошло».** Кнопка 1С запускает синхронный серверный вызов,
   на время которого страница не отвечает; наивная проверка в этот момент врёт «клик не дошёл».
3. **Запасной вход — горячая клавиша кнопки** (драйвер берёт её из двойника в меню «Еще»),
   с предварительным клик-фокусом в окно: без реального клика клавиши в 1С не доходят.

Подробный разбор и признаки опознания окон — `FIELD_NOTES.md`, раздел `1c-modal-windows`.
Эталоны — `templates/`.
